...
switch ($_GET["op"]) {
	case 'guardaryeditar':
		...
		$objusuariomodelo->setIdusuario($idusuario);
		$objusuariomodelo->setNombre($nombre);
		$objusuariomodelo->setTipo_documento($tipo_documento);
		$objusuariomodelo->setNum_documento($num_documento);
		$objusuariomodelo->setDireccion($direccion);
		$objusuariomodelo->setTelefono($telefono);
		$objusuariomodelo->setEmail($email);
		$objusuariomodelo->setCargo($cargo);
		$objusuariomodelo->setLogin($login);
		$objusuariomodelo->setClave($util->encrypt_decrypt('encrypt', $clave));
		...
		$objusuariomodelo->setImagen($imagen);

		//Hash SHA256 para la contraseña
		// $clavehash = hash("SHA256", $clave);
		if (empty($idusuario)) {
			$rspta = $usuario->RegistrarUsuario($objusuariomodelo, $_POST['permiso']);
			echo $rspta ? "Datos registrados correctamente" : "No se pudo registrar todos los datos del usuario";
		} else {
			// $rspta = $usuario->editar($idusuario, $nombre, $tipo_documento, $num_documento, $direccion, $telefono, $email, $cargo, $login, $clavehash, $imagen, $_POST['permiso']);
			$rspta = $usuario->ActualizarUsuario($objusuariomodelo, $_POST['permiso']);
	...
	case 'desactivar':
		$objusuariomodelo->setIdusuario($idusuario);
		$rspta = $usuario->DesactivarUsuario($objusuariomodelo);
		echo $rspta ? "Datos desactivados correctamente" : "No se pudo desactivar los datos";
		break;

	case 'activar':
		$objusuariomodelo->setIdusuario($idusuario);
		$rspta = $usuario->ActivarUsuario($objusuariomodelo);
	...
	case 'mostrar':
		...
		$rspta = $usuario->BuscarUsuario($objusuariomodelo);
		$rspta["clave"] = $util->encrypt_decrypt('decrypt', $rspta["clave"]);
	...
	case 'permisos':
		...
		require_once "../repositorio/PermisoRepository.php";
		$permiso = new Permiso();
		$rspta = $permiso->listar();
		//obtener permisos asigandos
		$id = $_GET['id'];
		$marcados = $usuarioPermiso->ListarPermisos($id);
		$valores = array();
	...
	case 'verificar':
		...
		// $objusuariomodelo = new UsuarioModelo();
		$objusuariomodelo->setLogin($logina);
		// $objusuariomodelo->setClave($clavehash);
